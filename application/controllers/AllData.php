...
public function upLink() {
  ...
  $cek = $this->db->select('att')->from('dt_control')->where('att',$file_name)
  ->get()->num_rows();
  if (!$cek) {
...
function addItemNo() {
  ...
  if (!$cek->item_no) {
    ...
    function delfile() {
      ...
      if (unlink("./assets/pdf/$name")) {
        $this->M_AllData->delfile($idd);
        echo '<script>alert("Success to delete file");</script>';
        redirect (site_url('alldata'), 'refresh');
      } else {
        echo '<script>alert("Filed to delete file");</script>';
      }
